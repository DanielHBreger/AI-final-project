...
|---|---------|------|---------|
| 8 | `python -u compare_architectures.py --no-vel --no-B --log results/ablation_novelB_<date>.json > logs/run_novelB.log` | ~1 h (per runs 1–2 timing) | **DONE 2026-07-07** → `results/ablation_novelB_20260707.json` (log `logs/run_novelB.log`). Importance prediction confirmed for trees, refuted for the MLP: XGBoost unchanged (pointwise 0.952 vs 0.950; +S(+W) 0.963/RMSE 0.372 vs 0.963/0.374, mass 0.86–1.00); MLP destabilises in log space (mlp_wide_sp_w 0.981→0.808±0.241, interior folds to 0.29–0.67) though its R2_mol (0.987) and mass (0.93–1.03) hold; headline `stacked_weighted_mwcal` retains R² 0.975±0.011, RMSE 0.340 (+16 %), mass 0.97–1.09, R2_mol 0.989 (Ridge leans on the tree branch, w_xgb≈0.75). §5.3 hedge replaced with the evaluated result in both paper.tex and paper_short.tex ("deployable" mislabel dropped — the six-feature set includes f_H2) |
...
|---|---------|------|------------------|
| 12 | Density-weight α sensitivity: **small code change first** (expose `_compute_weights` α — e.g. `--weight-alpha` in compare_architectures, or a standalone XGB-only sweep script), then α ∈ {10, 100, 1000} on `xgb_standard_sp_w` | ~1 h total (XGB-only) | **DONE 2026-07-07** → new standalone `weight_alpha_sweep.py` (XGB+spatial LOO, α passed to `_compute_weights`) → `results/weight_alpha_20260707.json` (log `logs/run12_alpha_sweep.log`). α=100 reproduces run-1b `xgb_standard_sp_w` R² **bit-for-bit** (max\|Δ\|=0.0). Flat response: mean R² 0.9631/0.9631/0.9632, RMSE 0.374/0.374/0.374, scatter 0.253 for α=10/100/1000 — only the MASS budget moves, and only at α=10 (worst-fold mass 0.62 vs 0.89 at α=100; α=1000 ≈ α=100). Adopted α=100 is on the flat part. §4.2 sensitivity sentence added; ChatGPT #6 closed. DESIGN_DECISIONS §4.1 |
...
| 21 | Full SHA-256 fingerprint of each data CSV in `compute_data_checksum` | **DONE 2026-07-07** — added `sha256_full` (whole-file hash) alongside the existing `sha256_partial` (first 8KB), computed in one pass per file; old logs' `sha256_partial` key stays valid, nothing invalidated. Smoke-tested: ~1.7 s for all 7 cubes (2.8 GB) |
...
| 23 | Fair U-Net final refit: use inner-val to pick the epoch count, discard that model, retrain fresh on all 6 outer-training cubes for exactly that many epochs, evaluate once on the held-out cube (standard model-selection/final-refit split — gives the U-Net the same 6-cube training data as the tabular models) | Reclassified 2026-07-07 after ChatGPT's round-2 rebuttal (docs/CHATGPT_CODE_REVIEW_DISPOSITIONS.md, "Reclassified after reconsideration"): my original objection wrongly conflated this with reusing the held-out cube. Legitimate improvement, not implemented — changes the CNN training entry point and needs a fresh ~4.6 h run |
