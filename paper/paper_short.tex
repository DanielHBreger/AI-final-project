...
nested and scores slightly \emph{higher}. Other details also differ
between the two runs, so this does not isolate nesting as the sole
cause; it shows only that the stricter protocol does not contradict
the comparison-table numbers.
...
magnetic-field components are negligible --- a rerun without them
leaves XGBoost unchanged and the calibrated stack at
\(\Rsq = 0.975\) with its mass budget closed, though the MLP branch
destabilises in log space). The complementary bound
